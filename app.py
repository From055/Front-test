# 파일명: app.py
# 기능: Render 배포 환경의 CORS 오류를 해결한 최종본
...
# <<<<<<<<<<<<<<<< 여기가 핵심! 허용할 웹페이지 주소를 정확히 알려줌 >>>>>>>>>>>>>>>>
# 이제 우리 서버는 이 주소에서 오는 요청만 안전하다고 판단하고 허용할 거야.
cors = CORS(app, resources={
  r"/api/*": {
    "origins": "https://front-test-1-7vkn.onrender.com"
  }
})
...
try:
    ...
    all_dfs = []
    market_list = {
        'KOSPI': 'KOSPI', 'KOSDAQ': 'KOSDAQ', 'ETF': 'ETF-KR', 
...
def resample_and_calculate_returns(price_df, timeframe):
    if timeframe == 'D': return price_df['Close'].pct_change() * 100
...
def get_stock_data():
    ...
    effective_start_date = (start_date_dt - timedelta(days=10)).strftime('%Y-%m-%d')
    themed_returns_data = {}
    stock_level_returns = []
            ...
            try:
                ...
                if not price_df.empty:
                    ...
                    if timeframe == 'D':
                        ...
                        stock_df = pd.DataFrame({'date': daily_return.index.strftime('%Y-%m-%d'), 'stock_name': f"{stock_name} ({code})", 'value': daily_return.round(2)}).dropna()
    ...
    correlation_df = pd.DataFrame(themed_returns_data).loc[start_date:]
    correlation_matrix = correlation_df.corr().fillna(0)
    ...
    for theme_name, return_series in themed_returns_data.items():
        ...
        theme_df = pd.DataFrame({'date': filtered_series.index.strftime('%Y-%m-%d'), 'sector': theme_name, 'value': filtered_series.round(2)})
